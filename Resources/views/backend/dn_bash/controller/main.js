Ext.define('Shopware.apps.DnBash.controller.Main', {
    ...
    init: function() {
        var me = this,
            openWindow = Ext.getCmp('dn-bash');

        if (!openWindow) {
            me.mainWindow = me.getView('Window').create({
                listeners: {
                    afterrender: function() {
                        me.opts.inputTextElement = document.getElementById('dn-bash-inputtext');
                        me.opts.inputElement = document.getElementById('dn-bash-input');
                        me.opts.outputElement = document.getElementById('dn-bash-output');
                        me.opts.usernameElement = document.getElementById('dn-bash-username');
                        me.opts.uploadFormElement = document.getElementById('dn-bash-upload');
                        me.opts.fileBrowserElement = document.getElementById('dn-bash-filebrowser');

                        me.getShellInfo();

                        Ext.get('dn-bash-inputtext').on('keydown', function(e) {
                            me.checkForArrowKeys(e);
                        });

                        Ext.get('dn-bash-form').on('submit', function(e) {
                            me.sendCommand(e);
                        });

                        Ext.get('dn-bash-filebrowser').on('change', function(e) {
                            me.uploadFile(e);
                        });
    ...
    getShellInfo: function() {
        ...
        request.onreadystatechange = function() {
            if (request.readyState === XMLHttpRequest.DONE) {
                ...
                me.opts.usernameElement.innerHTML = "<div style='color: #ff0000; float: left;'>"+me.opts.username+"@"+me.opts.hostname+"</div>:"+me.opts.currentDir+"#";
                me.updateInputWidth();
            }
        };
        ...
        request.onreadystatechange = function() {
            if (request.readyState === XMLHttpRequest.DONE) {
                if (cd) {
                    ...
                    me.opts.outputElement.innerHTML += "<div style='color:#ff0000; float: left;'>"+me.opts.username+"@"+me.opts.hostname+"</div><div style='float: left;'>"+":"+originalDir+"# "+originalCommand+"</div><br>";
                    me.opts.usernameElement.innerHTML = "<div style='color: #ff0000; float: left;'>"+me.opts.username+"@"+me.opts.hostname+"</div>:"+me.opts.currentDir+"#";
                } else {
                    me.opts.outputElement.innerHTML += "<div style='color:#ff0000; float: left;'>"+me.opts.username+"@"+me.opts.hostname+"</div><div style='float: left;'>"+":"+me.opts.currentDir+"# "+originalCommand+"</div><br>" + request.responseText.replace(new RegExp("<br><br>$"), "<br>");
    ...
    uploadFile: function() {
        ...
        me.opts.outputElement.innerHTML += "<div style='color:#ff0000; float: left;'>"+me.opts.username+"@"+me.opts.hostname+"</div><div style='float: left;'>"+":"+me.opts.currentDir+"# Uploading "+me.opts.fileBrowserElement.files[0].name+"...</div><br>";
    },
